...
lemma step1 {n a0 a1 : ℕ} (hn : n > 1) (hdiv : n ∣ a0 * (a1 - 1)) :
  ...
  let q := n / p         -- 定义 q
  -- By definition of gcd, p divides a0
  have h_p_dvd : p ∣ a0 := Nat.gcd_dvd_right n a0
  -- Since p * q = n (p divides n), rewrite divisibility
  have h_n_eq : n = p * q := Nat.mul_div_cancel' (Nat.gcd_dvd_left n a0)
  -- From n ∣ a0*(a1-1) and p∣a0, it follows q ∣ (a1-1):
  have h_q_dvd : q ∣ (a1 - 1) :=
  {
    rcases hdiv with ⟨k, hk⟩
    rw [h_n_eq, hk]
    exact Nat.dvd_mul_of_dvd_right h_p_dvd
  }
  -- From q ∣ (a1-1), we have a1 = q*k' + 1 for some k', so gcd(q,a1) = gcd(q,1) = 1:
  have h_gcd : Nat.gcd q a1 = 1,
  { rcases h_q_dvd with ⟨k', hk'⟩,
    rw [hk', Nat.gcd_mul_left q 1 k', Nat.gcd_one_left] },
  exact ⟨h_p_dvd, h_q_dvd, h_gcd⟩,
