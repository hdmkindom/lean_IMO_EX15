...
lemma step1 {n a0 a1 : ℕ} (hn : n > 1) (hdiv : n ∣ a0 * (a1 - 1)) :
  ...
  have h_a0_eq_p_r : ∃ r, a0 = p * r := h_p_dvd
  let ⟨r, hr⟩ := h_a0_eq_p_r
  -- proof : q ∣ (a1 - 1)

  have h_gcd_q_r : Nat.gcd q r = 1 := by
    have h_pq := h_n_eq.symm -- n = p * q
    have h_pr := hr.symm     -- a0 = p * r
  -- 从 n = p * q 和 a0 = p * r 开始
    rw [h_n_eq, hr]
    -- 使用 gcd 的性质
    rw [Nat.gcd_mul_left]
    -- 因为 p 和 q 互质，且 p 和 r 互质
  ...
  have h_gcd_q_k_eq_one : Nat.gcd q r = 1 := by
    rw [← Nat.gcd_mul_left p, hr, h_n_eq, h_p_div_n_a0]
    exact Nat.gcd_div_dvd_left _ _
