...
example (a b : ℕ) (h : b ∣ a) : ∃ k, a = b * k :=
  Nat.dvd_iff_exists_eq_mul.mp h

example (a b : ℕ) (h : b ∣ a) : ∃ k, a = b * k :=
  rcases Nat.dvd_iff_exists_eq_mul.mp h with ⟨k, hk⟩

example (a b : ℕ) (h : b ∣ a) : ∃ k, a = b * k :=
  obtain ⟨k, hk⟩ := Nat.dvd_iff_exists_eq_mul.mp h,
  ⟨k, hk⟩
/--
设 `n` 为正整数，`k ≥ 2`。
`a : Fin k → ℕ` 满足：
...
lemma step1 {n a0 a1 : ℕ} (hn : n > 1) (hdiv : n ∣ a0 * (a1 - 1)) :
  ...
  have h_n_eq : n = p * q := by
    rw [← Nat.mul_div_cancel' (Nat.gcd_dvd_left n a0)]
  have h_p_div_n_a0 : Nat.gcd n a0 = p := rfl
  have h_n_dvd_mul : n ∣ a0 * (a1 - 1) := hdiv

  have h_a0_eq_p_r : ∃ r, a0 = p * r := Nat.dvd_iff_exists_eq_mul.mp h_p_dvd
  let ⟨r, hr⟩ := h_a0_eq_p_r
  -- proof : q ∣ (a1 - 1)
  have h_gcd_q_k_eq_one : Nat.gcd q r = 1 := by
    rw [← Nat.gcd_mul_left p, hk, h_n_eq, h_p_div_n_a0]
    exact Nat.gcd_div_dvd_left _ _
  have h_q_dvd_a1_sub_1 : q ∣ (a1 - 1) :=
    Nat.dvd_of_gcd_eq_one_left_of_dvd h_gcd_q_k_eq_one h_q_dvd_r_mul
  have h_gcd_q_a1 : Nat.gcd q a1 = 1 := by
    rw [Nat.gcd_comm, Nat.gcd_rec]
    exact h_q_dvd_a1_sub_1
  exact ⟨h_p_dvd, h_q_dvd_a1_sub_1, h_gcd_q_a1⟩
