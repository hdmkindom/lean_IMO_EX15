...
lemma coprime_mul_dvd
  ...
  obtain ⟨c, hc⟩ := hn_div
  rw [hnpq] at hc
  have hp_dvd_xy : p ∣ x * y := by
    rw [hc]  -- 将 x * y 替换为 p * q * c
    -- 因为 p ∣ p * c，所以 p ∣ p * q * c
    apply dvd_mul_of_dvd_left
    apply dvd_mul_right
  have hq_dvd_xy : q ∣ x * y := by
    rw [hc]
    rw [mul_comm p q]
    apply dvd_mul_of_dvd_left
    apply dvd_mul_right
...
lemma coprime_mul_dvd2
  ...
  ∃ (x1 x2 : ℕ), p ∣ x1 ∧ q ∣ x2 ∧ x * y = x1 * x2 := by
    ...
    obtain ⟨t, ht⟩ := hn_div,  -- 从 n | x * y 推出 x * y = n * t

    have h1 : x * y = p * q * t := by rw [hnpq]; exact ht  -- 用 n = p * q 代入 x * y = n * t
